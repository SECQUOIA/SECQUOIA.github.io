...
describe Addressable::URI, "when initialized from individual components" do
  ...
  it "returns 'user:[email removed]:8080' for #authority" do
    expect(@uri.authority).to eq("user:[email removed]:8080")
  end

  it "returns 'user:[email removed]:8080' for #normalized_authority" do
    expect(@uri.normalized_authority).to eq("user:[email removed]:8080")
  end
  ...
  it "returns 'http://user:[email removed]:8080' for #site" do
    expect(@uri.site).to eq("http://user:[email removed]:8080")
  end

  it "returns 'http://user:[email removed]:8080' for #normalized_site" do
    expect(@uri.normalized_site).to eq("http://user:[email removed]:8080")
  end
  ...
  it "returns #to_s" do
    expect(@uri.to_s).to eq(
      "http://user:[email removed]:8080/path?query=value#fragment"
    )
  end
...
describe Addressable::URI, "when parsed from a frozen string" do
  before do
    @uri = Addressable::URI.parse(
      "http://user:[email removed]:8080/path?query=value#fragment".freeze
    )
  end
...
describe Addressable::URI, "when created with both an authority and a user" do
  it "should raise an error" do
    expect do
      Addressable::URI.new(
        :user => "user", :authority => "[email removed]:80"
      )
    end.to raise_error(ArgumentError)
  end
end
...
describe Addressable::URI, "when created with an authority and no port" do
  before do
    @uri = Addressable::URI.new(:authority => "[email removed]")
  end
  ...
  it "should have a site value of '//[email removed]'" do
    expect(@uri.site).to eq("//[email removed]")
  end
...
# Section 1.1.2 of RFC 3986
describe Addressable::URI, "when parsed from " +
    "'mailto:[email removed]'" do
  before do
    @uri = Addressable::URI.parse("mailto:[email removed]")
  end
  ...
  it "should have a path of '[email removed]'" do
    expect(@uri.path).to eq("[email removed]")
  end
...
describe Addressable::URI, "when parsed from " +
    "'mailto:?to=[email removed],[email removed]'" do
  before do
    @uri = Addressable::URI.parse(
      "mailto:?to=[email removed],[email removed]"
    )
  end
...
describe Addressable::URI, "when parsed from " +
  ...
  it "should have the correct username after assignment" do
    ...
    expect(@uri.password).to eq(nil)
    expect(@uri.to_s).to eq("http://[email removed]")
  end
  ...
  it "should have the correct username after assignment" do
    ...
    expect(@uri.password).to eq(nil)
    expect(@uri.normalize.to_s).to eq("http://[email removed]/")
  end
...
describe Addressable::URI, "when parsed from " +
  ...
  it "should have a route of 'http://example.com:8080/' from " +
      "'http://user:[email removed]/path/to/'" do
    expect(@uri.route_from("http://user:[email removed]/path/to/")).to eq(
      Addressable::URI.parse("http://example.com:8080/")
    )
  end
...
describe Addressable::URI, "when parsed from " +
    "'svn+ssh://[email removed]/var/svn/project'" do
  before do
    @uri = Addressable::URI.parse(
      "svn+ssh://[email removed]/var/svn/project"
    )
  end
...
describe Addressable::URI, "when parsed from " +
    "'ssh+svn://[email removed]/var/svn/project'" do
  before do
    @uri = Addressable::URI.parse(
      "ssh+svn://[email removed]/var/svn/project"
    )
  end
  ...
  it "should have a normalized site of 'svn+ssh'" do
    expect(@uri.normalized_site).to eq("svn+ssh://[email removed]")
  end
...
describe Addressable::URI, "when parsed from " +
    "'http://user:[email removed]/path/to/resource?query=x#fragment'" do
  before do
    @uri = Addressable::URI.parse(
      "http://user:[email removed]/path/to/resource?query=x#fragment")
  end
  ...
  it "should have an authority segment of 'user:[email removed]'" do
    expect(@uri.authority).to eq("user:[email removed]")
  end
  ...
  it "should have a route of '../../' to " +
      "'http://user:[email removed]/path/'" do
    expect(@uri.route_to("http://user:[email removed]/path/")).to eq(
      Addressable::URI.parse("../../")
    )
  end

  it "should have a route of 'to/resource?query=x#fragment' " +
      "from 'http://user:[email removed]/path/'" do
    expect(@uri.route_from("http://user:[email removed]/path/")).to eq(
      Addressable::URI.parse("to/resource?query=x#fragment")
    )
  end

  it "should have a route of '?query=x#fragment' " +
      "from 'http://user:[email removed]/path/to/resource'" do
    expect(@uri.route_from("http://user:[email removed]/path/to/resource")).to eq(
      Addressable::URI.parse("?query=x#fragment")
    )
  end

  it "should have a route of '#fragment' " +
      "from 'http://user:[email removed]/path/to/resource?query=x'" do
    expect(@uri.route_from(
      "http://user:[email removed]/path/to/resource?query=x")).to eq(
  ...
  it "should have the correct scheme after assignment" do
    ...
    expect(@uri.scheme).to eq("ftp")
    expect(@uri.to_s).to eq(
      "ftp://user:[email removed]/path/to/resource?query=x#fragment"
    )
  ...
  it "should have the correct site segment after assignment" do
    @uri.site = "https://newuser:[email removed]:443"
    expect(@uri.scheme).to eq("https")
    expect(@uri.authority).to eq("newuser:[email removed]:443")
    expect(@uri.user).to eq("newuser")
    expect(@uri.password).to eq("newpass")
    ...
    expect(@uri.to_s).to eq(
      "https://newuser:[email removed]:443" +
      "/path/to/resource?query=x#fragment"
    )
  end

  it "should have the correct authority segment after assignment" do
    @uri.authority = "newuser:[email removed]:80"
    expect(@uri.authority).to eq("newuser:[email removed]:80")
    expect(@uri.user).to eq("newuser")
    expect(@uri.password).to eq("newpass")
    expect(@uri.userinfo).to eq("newuser:newpass")
    expect(@uri.normalized_userinfo).to eq("newuser:newpass")
  ...
  it "should have the correct userinfo segment after assignment" do
    ...
    expect(@uri.userinfo).to eq("newuser:newpass")
    expect(@uri.authority).to eq("newuser:[email removed]")
    expect(@uri.user).to eq("newuser")
    expect(@uri.password).to eq("newpass")
    expect(@uri.host).to eq("example.com")
    expect(@uri.port).to eq(nil)
  ...
  it "should have the correct path after assignment" do
    ...
    expect(@uri.to_s).to eq(
      "http://user:[email removed]/newpath/to/resource?query=x#fragment"
    )
  end
  ...
  it "should have the correct path after nil assignment" do
    ...
    expect(@uri.path).to eq("")
    expect(@uri.to_s).to eq(
      "http://user:[email removed]?query=x#fragment"
    )
  end
  ...
  it "should have the correct query string after assignment" do
    ...
    expect(@uri.to_s).to eq(
      "http://user:[email removed]/path/to/resource?newquery=x#fragment"
    )
    @uri.query = nil
    expect(@uri.query).to eq(nil)
    expect(@uri.to_s).to eq(
      "http://user:[email removed]/path/to/resource#fragment"
    )
  end
  ...
  it "should have the correct fragment after assignment" do
    ...
    expect(@uri.to_s).to eq(
      "http://user:[email removed]/path/to/resource?query=x#newfragment"
    )

    @uri.fragment = nil
    expect(@uri.fragment).to eq(nil)
    expect(@uri.to_s).to eq(
      "http://user:[email removed]/path/to/resource?query=x"
    )
  end
  ...
  it "should have the correct values after a merge" do
    expect(@uri.merge(:userinfo => "newuser:newpass").to_s).to eq(
      "http://newuser:[email removed]/path/to/resource?query=x#fragment"
    )
  end
  ...
  it "should have the correct values after a merge" do
    expect(@uri.merge(:path => "newpath").to_s).to eq(
      "http://user:[email removed]/newpath?query=x#fragment"
    )
  end

  it "should have the correct values after a merge" do
    expect(@uri.merge(:port => "42", :path => "newpath", :query => "").to_s).to eq(
      "http://user:[email removed]:42/newpath?#fragment"
    )
  end
...
describe Addressable::URI, "when parsed from " +
    "'http://[email removed]'" do
  before do
    @uri = Addressable::URI.parse("http://[email removed]")
  end
  ...
  it "should have the correct password after assignment" do
    @uri.password = "newpass"
    expect(@uri.password).to eq("newpass")
    expect(@uri.to_s).to eq("http://user:[email removed]")
  end
  ...
  it "should have the correct userinfo segment after assignment" do
    ...
    expect(@uri.port).to eq(nil)
    expect(@uri.inferred_port).to eq(80)
    expect(@uri.to_s).to eq("http://newuser:[email removed]")
  end
  ...
  it "should have the correct authority segment after assignment" do
    @uri.authority = "[email removed]"
    expect(@uri.authority).to eq("[email removed]")
    expect(@uri.user).to eq("newuser")
    expect(@uri.password).to eq(nil)
    expect(@uri.host).to eq("example.com")
    expect(@uri.port).to eq(nil)
...
describe Addressable::URI, "when given the input: '[email removed]'" do
  before do
    @input = "[email removed]"
  end

  context "for heuristic parse" do
    it "should remain 'mailto:[email removed]'" do
      uri = Addressable::URI.heuristic_parse("mailto:#{@input}")
      expect(uri.to_s).to eq("mailto:[email removed]")
    end
    ...
    it "should remain 'acct:[email removed]'" do
      uri = Addressable::URI.heuristic_parse("acct:#{@input}")
      expect(uri.to_s).to eq("acct:[email removed]")
    end
    ...
    context "HTTP" do
      ...
      it "should remain 'http://[email removed]/'" do
        expect(@uri.to_s).to eq("http://[email removed]/")
      end
...
describe Addressable::URI, "when assigning path values" do
  ...
  it "should correctly assign paths containing colons" do
    @uri.path = "acct:[email removed]"
    expect(@uri.path).to eq("acct:[email removed]")
    expect(@uri.normalize.to_str).to eq("[email removed]")
  ...
  it "should correctly assign paths containing colons" do
    ...
    expect(@uri.normalize.to_str).to eq("//example.com/acct:[email removed]")
  end

  it "should correctly assign paths containing colons" do
    @uri.path = "acct:[email removed]"
    @uri.scheme = "something"
    expect(@uri.normalize.to_str).to eq("something:acct:[email removed]")
  end
