---
layout: mdpage
title: PhD Handbook
permalink: /handbook/
description: 'A guide for PhD students in the SECQUOIA group'
image: assets/images/books.webp
nav-menu: false
show_tile: false
banner_color: style5
---

<!-- markdownlint-disable MD033 -->

This is not Purdue's official graduate handbook. That document covers the formal requirements: qualifying exams, committee milestones, dissertation deadlines. This handbook covers something different: how SECQUOIA actually works, what we expect of each other, and the things that matter but that nobody writes down.

It is written primarily for PhD students, but much of it applies to everyone in the group.

**If you are in your first week:** read [Getting started](#getting-started-your-first-30-days), [The weekly rhythm](#the-weekly-rhythm), and [Communication](#communication) first. The rest can wait until it becomes relevant.

---

---

## What is a PhD, really?

A PhD is a training in original research. By the end, you should be able to identify a problem worth solving, develop a method to address it, and communicate what you found to a community that cares. The degree is a byproduct of that process.

At SECQUOIA, that contribution typically sits at the intersection of theory, computation, and practice: developing new algorithms or mathematical insights, implementing them as reliable software, and demonstrating their value on real engineering problems. The specific fields are optimization, quantum computing, and process systems engineering. The work spans all three, and learning to connect those layers is part of what makes a SECQUOIA PhD distinctive.

---

## David's role and yours

David's job is to secure the funding that makes your work possible and to inspire and frame ideas at the level of research direction and contribution. He is not here to manage your daily schedule or tell you what to do each morning.

Your job is to become an independent researcher. That means owning your project, driving it forward when things are unclear, and asking for help when you are stuck. Struggling alone for weeks is not independence; it is a cost to you and to the group. On funding: David's job is to keep the lights on. Your job is to do good work.

This division is intentional. Dependence does not prepare you for what comes after SECQUOIA.

---

## Getting started: your first 30 days

A new student's first month has three priorities before research begins in earnest.

**Set up your environment.** Get access to the SECQUOIA GitHub organization, Slack workspace, and any computing resources relevant to your project. If you do not have access to something you need, ask.

**Meet everyone in the group.** Schedule a short conversation with each current lab member: understand what they work on and who they are. These are not formal meetings. These relationships matter more than most students expect.

**Read the papers David assigns.** At the start, David will point you toward a handful of foundational papers relevant to your project. Read them carefully and come to your next meeting with questions.

Then, before you write a single line of code or prove a single theorem: test your idea in the worst case. What would have to be true for this approach to fail? What assumptions is the whole thing resting on? A week of stress-testing a hypothesis before building anything can save months of work later.

---

## The weekly rhythm

A typical week includes:

- **Group meeting:** exists for accountability, cross-pollination of ideas, and building a shared understanding of what the group is working on
- **Weekly 1-on-1 with David:** your dedicated time to discuss your project, blockers, and direction
- **TAing:** teaching a concept well requires understanding it deeply, and that depth carries back into your research
- **Advising an undergraduate:** mentoring is part of your role in the community
- **PSE seminar:** staying connected to the broader field
- **Classes** (especially in your first two years)
- **Research:** the rest is yours to drive

When you and David are both available and on campus, meetings are in person. Two days per week in the office is the minimum; being present in the lab matters for the group and for your own development.

### Preparing for your 1-on-1

Keep a running shared document that you update before each meeting. It does not need to be long, but it should answer three things: what you worked on since the last meeting, where you are stuck, and what you need to decide or discuss. This document becomes a record of your progress over time and makes the meeting more useful for both of you.

---

## Communication

**Day to day:** The group uses Slack for async communication within the lab. Respond to messages same day during working hours. Use email for anything that needs a written record and for communication outside the group: collaborators, faculty elsewhere, conference organizers. Code, issues, and project tracking live on GitHub under the SECQUOIA organization. Collaborative writing happens in Overleaf.

**When you are struggling:** Be direct. There is no single right way to tell David or your colleagues that things are hard. What will cost you is staying silent. Struggling on your own for weeks costs you time you cannot get back, and it costs the group the chance to help. No one is expected to have everything figured out. Everyone is expected to communicate.

---

## Research: how we approach it

### Selecting meaningful research problems

Before committing time to a direction, it helps to pressure-test it with questions like these (adapted from Eric Gilbert):

**On impact:**
- What difference will this work make if it succeeds?
- Why is this one of the most important open problems in its area?
- Whose engineering systems, decisions, or tools will work better because of this?
- Will it inspire a new class of algorithms, methods, or software?

**On whether anyone will care:**
- Will other researchers cite it and build on it?
- Will engineers or scientists actually use the methods or tools that come out of it?
- Will it matter 10 or 20 years from now, when hardware and software have changed significantly?

**On novelty and positioning:**
- If you don't do this, who else might, and how soon?
- What makes you and SECQUOIA uniquely positioned to do it well?
- How does it advance beyond what existing solvers, algorithms, or frameworks already do?

Not all ideas will hold up under these questions, and that is fine. Working through them together is part of the research process — it helps narrow the space toward ideas that are both feasible and worth doing. David recommends Hamming's lecture on [You and Your Research](https://www.cs.virginia.edu/~robins/YouAndYourResearch.html) as a useful companion on this.

### Testing ideas

Start from the hardest case. If your method cannot handle the corner case, you want to know that before you have built everything around it.

### Dead ends

If you believe a direction is not working, bring it to David. The two of you evaluate it together over a period calibrated to where you are in the PhD and how much it would cost to pivot. A first-year can change direction relatively easily; a fourth-year cannot. The decision to continue or change course is made jointly, driven by evidence, not by calendar or stubbornness.

### Disagreements

If you think David is wrong about something, say so with arguments. Respectful disagreement is expected and welcomed.

### Writing and publishing

Start writing when you have preliminary results worth communicating. The right time to bring a result to David is when you find it interesting, not when it is polished. Students wait too long because they want things to be ready; bring it early. Before drafting, have a conversation with David about what the paper is, who it is for, and what form it should take (journal article, conference paper, extended abstract). That conversation shapes everything about how to write it. Once that is clear, agree with David on how the drafting will work: David outlines the narrative for some papers, while you draft others independently first. The process depends on the student and the paper. What matters is agreeing on the approach before you start.

**Internal deadlines:** For paper submissions, the paper must be in submittable state two weeks before the external deadline. For presentations, slides must be in presentable state one week before. If a paper is not ready two weeks out, we do not submit it to that deadline — we plan for the next one. This policy exists to protect quality and to give everyone enough time to give and receive useful feedback.

### Quality and pace

By the end of your PhD, you should aim to have 3–4 core papers you are genuinely proud of. These will form the backbone of your job talk if you pursue academic or research positions. A useful rough strategy:

- Aim for one high-risk, high-reward paper per year. Some will not get accepted immediately, or will not work out at all. That is the nature of ambitious work.
- Alongside that, consider a lower-risk, more straightforward paper every couple of years — one that is still good, but where the outcome is more predictable.
- Optionally, support a collaborator's project where you contribute meaningfully but are not the lead. This should take less than half a day per week.

This typically means being involved in two to three projects at any given time. Working on more than that in parallel is usually counterproductive. If you are submitting at least one first-authored paper per year after your first year, that is a reasonable sign of progress. If a full year passes without a submission, that is worth reflecting on together.

Quality and speed are in tension, but the answer is not to sacrifice one for the other — it is to plan earlier and scope more carefully.

### Code and software

SECQUOIA publishes open-source tools, which means your code has readers beyond your committee. The standard: version control everything in the SECQUOIA GitHub organization, write results that are reproducible from the repository, and document software that goes public. Code that underpins a published result should be something a reader can actually run.

---

## Authorship and credit

We follow clear conventions:

- **First author:** the person who did the most work on the paper
- **Co-first authorship:** used when more than one person made the primary contribution, noted explicitly in the paper
- **Senior and corresponding author:** David, listed last

These rules apply consistently. If you believe a contribution is being overlooked, bring it up directly.

---

## Conferences and presentations

David is a strong advocate for students attending and presenting at conferences. Getting your work in front of the community, and meeting the people whose papers you read, is a core part of the training.

**Contribution requirements to attend:**

| Conference type | First-year students | All others |
|---|---|---|
| Without proceedings | 1 contribution | 2 contributions |
| With proceedings | 1 accepted paper | 1 accepted paper |

Presenting before you graduate is expected. It is part of what "contribution to the community" means in this group.

---

## Research dissemination and visibility

Doing good work is necessary. Making sure the right people know about it is part of the job too.

David will actively look for opportunities for you to give talks — at conferences, workshops, seminars, and research groups at other institutions. When those opportunities arise, you will be pushed to the front. If press coverage ever comes from your work, the goal is for your name to be the one cited, not David's.

**LinkedIn:** Maintain a professional profile and share your publications and presentations when they come out. The optimization, quantum computing, and process systems communities are active there, and visibility compounds over time. A paper that sits in a repository and is never shared is harder to build on.

The basic loop is: do great work, notice opportunities to share it, take those opportunities, and go back to doing great work.

## Fellowships

Applying for fellowships during your PhD is worth the effort. They provide funding flexibility, reduce pressure on the lab to support your work through grants, and add meaningful recognition to your CV. The application process also forces you to articulate your research vision — which is useful regardless of outcome.

Fellowships relevant to SECQUOIA students include:

- **[NSF Graduate Research Fellowship (GRFP)](https://www.nsfgrfp.org/):** the primary US graduate fellowship; open to early-stage PhD students
- **[DOE Computational Science Graduate Fellowship (CSGF)](https://www.krellinst.org/csgf/):** highly relevant for SECQUOIA's focus on computational methods and scientific applications
- **Industry fellowships** (IBM, Google, and similar): provide both funding and industry connections; worth watching for annual calls

Talk to David early about fellowship opportunities — some have strict eligibility windows based on your year in the program.

## The valley of shit

At some point in your PhD, usually somewhere in the middle, you will hit a period where the work is hard, the end feels impossibly far away, and you are not sure you are capable of finishing. This period has a name: [the valley of shit](https://thesiswhisperer.com/2012/05/08/the-valley-of-shit/), a term coined by Inger Mewburn at *The Thesis Whisperer*.

It is real. It happens to nearly everyone. It does not mean you should quit.

What helps:

- **Remember the big picture.** Some parts of a PhD are genuinely unglamorous: debugging code you did not write, rerunning experiments, rewriting the same section five times. Within the larger arc of the work, those pieces add up to something that matters. Keep that picture in view.
- **Talk to your colleagues.** They are probably in a similar place, or they have been. The lab exists as a community partly for this reason.
- **Tell David.** There is no wrong time to have that conversation. David has been there; he will tell you that.
- **Use Purdue's professional support.** [Purdue's Counseling and Psychological Services (CAPS)](https://www.purdue.edu/caps/) exists for exactly this. Talking to a counselor when things are hard is not a sign of failure; it is good judgment. If you or someone you know is in crisis, the [988 Suicide and Crisis Lifeline](https://988lifeline.org/) (call or text 988) and the Crisis Text Line (text HOME to 741741) are available any time.

A note on what David can and cannot do: David will listen, help you think through what is going on, and work to create space in your schedule when that is within his power. He will direct you to people who are better equipped to help when needed. However, there are limits to confidentiality: if you share plans to seriously harm yourself or others, David is not able to keep that information private. He will tell you if a conversation has reached territory that requires involving others with the appropriate training or institutional authority.

---

## Research citizenship

Being a researcher means more than working on your own project. You are part of a community that functions through collective contribution.

In practice: review papers when asked, and do it well. Reviewing starts when you know enough about an area to have a useful opinion; typically not in your first year. Attend talks, including ones outside your immediate area. Help junior students in the group when they are stuck on something you understand. Show up for your colleagues' defenses, presentations, and milestones. Contribute to the group's shared software even when it is not your primary project.

These are not extras. They are part of what it means to belong to this field.

---

## Professionalism and conduct

When you interact with others in the lab, the department, at conferences, or on internships, you are representing SECQUOIA. Bring your best.

In professional settings — including informal gatherings with colleagues — the baseline is civility and respect. You are not required to like everyone, but you are required to treat them as colleagues.

Harassment, discrimination, bullying, and retaliation have no place in this group, in the department, or in the broader research community. These behaviors harm everyone involved and are taken seriously at every level. SECQUOIA members are expected to adhere to [Purdue's student code of conduct](https://www.purdue.edu/odos/osrr/student-code-of-conduct/) and the norms of the professional communities we belong to.

## Purdue's formal milestones

Purdue's PhD has four major formal checkpoints: the qualifying exam, dissertation committee formation, the proposal (preliminary) defense, and the final dissertation defense. Each has official requirements around timing and format. The authoritative source is the [Davidson School of Chemical Engineering graduate program](https://engineering.purdue.edu/ChE/academics/graduate); check there for current requirements.

David will help you navigate all of these. They are checkpoints, not the goal.

## Graduating from SECQUOIA

This section is about David's standard, above and beyond Purdue's requirements.

A SECQUOIA PhD means you have:

- Made a genuine, communicable contribution to the field: something new that the community can build on
- Published and presented that work so others know it exists and can use it
- Built enough of a record, in papers, software, presentations, and collaborations, to thrive wherever you want to go

One piece of work the field can build on is worth more than ten that disappear. The question is not how many papers, but whether your contributions have genuinely moved something forward. The goal is leaving ready, not crossing a minimum bar.

---

## The job market

Have a serious conversation with David about your plans by the end of your third year, regardless of which direction you are considering. That conversation shapes what you work on in your final years.

David writes letters of recommendation, helps you prepare your job talk through practice runs and feedback, and makes introductions in the community where they are useful. When offers or opportunities arrive, he works through them with you: whether the timing is right, whether the fit is real, whether it serves where you want to go.

Timelines depend on your path. Academic positions typically require going on the market in your fourth or fifth year, which means serious preparation starting in year three. Industry and national lab hiring can happen earlier and more continuously. Postdoctoral positions vary. None of these are rigid, but none can be improvised at the last minute. Start the conversation early.

---

## Unexpected opportunities

Summer internships at national labs, industry, or other universities are encouraged when the timing is right. When something arises during your PhD, an industry offer, a fellowship, a collaboration, an internship, bring it to David early. The right answer depends entirely on who you are, where you are in the program, and where you want to go. There is no standing rule that a given opportunity is automatically right or wrong.

SECQUOIA trains people for many paths: faculty positions, national labs, industry, startups, policy. All are supported.

---

## Vacations

Take them. Self-request your vacation and it will be approved. Rest is what makes sustained, rigorous work possible; it is not a reward for having done enough.

---

## External collaborations

When you work with people outside the lab, at other universities, national labs, or in industry, you are representing SECQUOIA. Keep David in the loop on the collaboration, its outputs, and how credit is being shared. This is how the group works as a unit.

---

<ul class="actions">
    <li><a href="/9-values.html" class="button icon fa-arrow-left">Back to Values</a></li>
    <li><a href="/#launch" class="button icon fa-home">Go back to launch page</a></li>
</ul>

---

*This handbook was written by David E. Bernal Neira with input from the SECQUOIA group. It is a living document; suggestions and corrections from current members are welcome.*

*It was written with inspiration from the [CoALA Lab PhD Handbook](https://docs.google.com/document/d/1C9hor13LKhIdO47n36rQdYem7c9PRdaMOOGX9IGUkEM/edit) by Prof. Ken Holstein at CMU. The concept of the "valley of shit" was coined by [Inger Mewburn](https://thesiswhisperer.com/2012/05/08/the-valley-of-shit/) at The Thesis Whisperer. We are grateful to both for making their thinking publicly available.*
